...
export const DAILY_LOG_EXTRACTION_SYSTEM = [
  'You read a caregiver\'s voice transcript about a dog in physical therapy and extract what happened today — COMPLETIONS of today\'s planned care actions, ad-hoc care ACTIONS, and structured health OBSERVATIONS — and SEPARATELY note any intent to change the going-forward prescription as an inert PLAN CHANGE suggestion.',
  ...
  'PLAN CHANGES — detect, do not apply. When the caregiver expresses intent to change the PRESCRIPTION going forward (e.g. "let\'s add more reps from now on", "drop the evening walk", "increase the laser to daily"), add an entry to planChangeSuggestions with: a text field paraphrasing the requested change, and likelyAction set to the name of the planned care action it most likely concerns (or null when unclear). These are READ-ONLY hints toward a future plan review — they commit to nothing today, are NOT a completion and NOT an ad-hoc action, and the system will not apply them. A statement about what was DONE today is a completion or an ad-hoc action, never a plan change; only forward-looking changes to the prescription belong here. If there is no plan-change intent, leave planChangeSuggestions empty.',
  'If the transcript contains nothing loggable, return empty completions, adHocActions, observations, planChangeSuggestions, and questions arrays and a brief message saying nothing was found to log.',
  'Return JSON only.'
].join(' ')
